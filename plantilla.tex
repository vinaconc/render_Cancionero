...
% Implementación personalizada del comando \diagram usando TikZ
\newcommand\diagram[2]{%
  \begin{center}
  \begin{tikzpicture}[
    box/.style={rectangle, draw=black, thick, minimum width=2cm, minimum height=1cm, align=center},
    content/.style={rectangle, draw=black, thick, minimum width=14cm, minimum height=1cm, align=left, text width=13.5cm}
  ]
    \node[box] at (0,0) {#1};
    \node[content] at (3,0) {#2};
  \end{tikzpicture}
  \end{center}
}

% Comando alternativo más simple sin TikZ (fallback)
\newcommand\diagramsimple[2]{%
  \noindent
  \fbox{#1} \hspace{0.5cm} \fbox{\parbox{13.5cm}{#2}}
}
